...
| 0.8 | 2026-08-14 | 阻断级修复：P14 部分成交后撤单/过期/取消的状态转移修正为 broker 终态 `cancelled/rejected/expired`（不再错误写成 `filled`），reservation 新增 `partially_settled` 终态并补齐 accepted/working→取消、零成交取消、部分成交后过期等全部转移路径；多租户共享 broker account 改为默认禁止，确需共享时使用「主账户 + 租户子额度」正式模型（数据库约束、额度分配、跨租户隔离测试），不再靠人工审批放行。高风险修复：状态转移改为数据库/统一 transition service 强制 + 状态转移白名单 + append-only 事件表 + 非法组合测试；unknown→not_submitted 增加对账证据、查询次数、时间窗口、broker 原始响应与审计字段；overrun 冲正改为 append-only adjustment event 并支持账户额度从事件账本重放；P15 与 P19A 历史迁移边界明确（paper 只由 P15 迁移，P19A 不重复导入，P15 加入 P19A 前置）；开发环境 insecure key 增加运行时防误用（禁止保存 broker/MFA 凭据并 fail closed）。 |

---

## 1. 目的与修订结论

本文件将 `MERGE_DESIGN.md` 拆分为可独立实施、验证、回滚和审批的 Plan。第八版在第七版基础上修正了部分成交后撤单/过期/取消的状态转移错误、把多租户共享券商账户从「人工审批例外」收紧为「默认禁止 + 正式主账户/租户子额度模型」，并为状态转移、对账证据、overrun 冲正、paper 历史迁移边界和开发密钥防误用补齐了强制约束。各版共同保留的核心方向：
...
53. 部分成交后撤单/过期/取消：broker 状态不再错误写成 `filled`，而是 `cancelled/rejected/expired`；已成交部分保留 `consumed`，未成交部分转 `released`；reservation 新增终态 `partially_settled`（consumed>0、released>0、remaining=0），并补齐 accepted/working→取消、零成交取消、部分成交后过期等全部转移路径。
54. 多租户共享 broker account 默认禁止；确需共享时使用「主账户 + 租户子额度」正式模型，所有租户订单进入全局账户级 reservation，增加数据库约束、额度分配和跨租户隔离测试，不再靠人工审批放行。

第八版同时落实的高风险修复：

55. 状态转移改为数据库/统一 transition service 强制：状态转移白名单、数据库 CHECK 或唯一 transition service、append-only 状态事件表、非法组合与并发转移测试。
56. unknown→not_submitted 增加 reconciliation evidence：最短等待时间、broker 最终一致性延迟窗口、查询次数、broker 原始响应与时间戳保存、以及哪些响应足以证明「未接单」。
57. overrun 冲正改为 append-only adjustment event（记录 broker fill/event id、原因、操作者、前后金额、审批人和证据），账户额度可从事件账本重放。
58. P15 与 P19A 边界明确：paper orders/fills/cash/positions 只由 P15 迁移，P19A 不重复导入、不覆盖 canonical paper ledger；P15 加入 P19A 前置。
59. 开发环境 insecure key 增加运行时防误用：insecure 模式禁止保存 broker credential 与 MFA secret，检测到写入即 fail closed，仅允许测试 fixture 或非敏感开发数据。
...
| P19A | 历史导入与切换候选 | 数据迁移完成、可切换候选态 | P7, P10, P15, P17（不依赖 P18/G5；paper 历史只由 P15 迁移） |
...
2. 禁止任何环境（含 staging/sandbox）在缺少持久化密钥时生成临时内存密钥用于券商凭据加密；开发环境如需临时密钥必须显式 `ALLOW_INSECURE_DEV_KEY=true`，并在运行时强制防误用：insecure key 模式**禁止保存 broker credential 与 MFA secret**，检测到任何 broker credential/MFA 写入请求即 fail closed；该模式仅允许测试 fixture、明文测试数据或其他明确的非敏感开发数据，且不得与真实券商账户、真实 MFA 绑定。
...
- insecure key 模式在运行时拒绝 broker credential/MFA 写入，相关 fail closed 测试通过。
...
5. 建立账户级 `AccountExposureReservationService`（D-013）：
   ...
   - 新增 broker account registry。账户边界使用券商返回的稳定身份 `broker + environment + broker_account_id`，不能只用 `credential_id`；多个 credential 指向同一真实账户时映射到同一 account key；原始 account id 按敏感字段保存，对外使用不可逆 hash/内部 UUID。**默认禁止任何跨租户共享同一真实 broker account**（D-013 账户级安全边界）。对既有数据执行账户注册迁移时，先扫描检测多租户/多用户共享同一真实账户，检测到即 fail closed（该账户禁开新仓、只允许对账与撤单）。确需共享的业务（如受控机构子账户、顾问-客户关系）**不得通过人工审批放行绕过**，必须实现正式的「主账户 + 租户子额度」模型：真实 broker account 作为唯一 `master_account`，每个租户持有显式 `tenant_sub_limit`（base currency + market/currency），所有租户订单仍进入全局 `master_account` 的账户级 reservation；数据库以唯一约束 + 外键强制租户子额度归属与合计不超过 master 限额，任何子额度变更走 append-only 事件并留审计。跨租户订单必须通过跨租户隔离测试，验证一个租户的订单不能影响其他租户的额度、risk lock 或持仓，且不能作为绕过租户数据隔离的入口。
...
8. execution gate、broker order 与 reservation 使用三个独立状态字段（D-015），禁止用一个 `pending/executed` 同时表达审批、券商接单和额度消费：
   ...
   | `reservation_status` | `reserved / submitting / active / unknown / partially_consumed / consumed / released / partially_settled / overrun_locked` |
   ...
   | broker 明确接受但零成交后被取消 | `submission_started` → `closed` | `accepted`/`working` → `cancelled` | `active` → `released` | 零成交取消，全额释放 |
   | broker 明确接受但零成交后过期 | `submission_started` → `closed` | `accepted`/`working` → `expired` | `active` → `released` | 零成交过期，全额释放 |
   | 部分成交后剩余撤单 | `submission_started` → `closed` | `partially_filled` → `cancelled` | `partially_consumed` → `partially_settled` | 成交部分 consumed，未成交部分 released |
   | 部分成交后剩余被拒绝 | `submission_started` → `closed` | `partially_filled` → `rejected` | `partially_consumed` → `partially_settled` | 成交部分 consumed，未成交部分 released |
   | 部分成交后剩余过期 | `submission_started` → `closed` | `partially_filled` → `expired` | `partially_consumed` → `partially_settled` | 成交部分 consumed，未成交部分 released |
   | 全部成交 | `submission_started` → `closed` | `filled` | `consumed` | 全额 consumed |
   | 成交超额（overrun） | `submission_started` → `closed` | `filled` | `consumed` → `overrun_locked` | 触发 risk lock；调整事件后转 `consumed`（额度补足）或 `released`（确认无超额） |
   | 整体终止（拒绝/取消/过期/完成） | `closed` | 终态 | 终态 | 三字段最终一致 |

   说明：
   - `partially_settled` 是部分成交的合法终态，表示 `consumed_exposure > 0` 且 `released_exposure > 0` 且 `remaining_reserved_exposure = 0`；部分成交后剩余撤单/拒绝/过期一律进入该终态，**不得把 broker 状态写成 `filled`**。
   - 全部成交的终态是 `consumed`（`remaining=0`、`released=0`）；零成交取消/拒绝/过期的终态是 `released`（`consumed=0`、`remaining=0`）。

   状态转移必须由数据库或统一服务强制，不能只依赖应用代码约定：

   - 状态转移白名单：把上表每个合法 `(from -> to)` 固化为一组允许的转移，非法组合（如 `closed + accepted`、`released + working`、`unknown + consumed`、`ready_to_submit + submitting`）必须在数据库事务层被拒绝；
   - 实现方式二选一：数据库 CHECK 约束 + 专用状态转移函数，或唯一 `TransitionService`（所有写路径只能通过它改状态），禁止绕过服务直接 UPDATE 状态字段；
   - 新增 append-only 状态事件表，记录每次状态/金额推进的 `event_id`、前后状态、触发事件、操作者、时间戳、幂等键和证据引用；主状态是事件重放的投影，不得反向修改历史事件；
   - 测试覆盖每一个合法转移、每一个非法组合的拒绝、并发转移竞争（两个 worker 同时推进同一订单）以及事件表与主状态的一致性重放校验。
...
12. reconciliation evidence 标准（`unknown -> not_submitted` 前必须满足，防止券商最终一致性延迟导致过早释放额度）：
   - 最短等待时间：至少经过 1 个对账扫描周期（30 秒）且不得早于 `submitting_at` 后该 broker 声明的最终一致性窗口（默认不低于 60 秒，可配置）；
   - 查询次数：至少连续 2 次独立 broker 查询（按 client_order_id）都返回「不存在该订单/未找到」且时间戳递增，才可作为未接单证据；
   - 证据留存：保存每次查询的 broker 原始响应、broker 侧时间戳、本地发起/收到时间、查询者、重试次数和 correlation id；
   - 仅当 broker 明确返回「未找到/未接收」才可推进为 `not_submitted`；broker 返回「处理中/无法确定/超时」继续保留 `unknown`，不得释放；
   - 释放动作本身也写入 append-only reconciliation event（记录证据引用、前后状态、操作者与时间戳），便于事后复核。
13. 市场单/可成交限价单按 worst-case protected price 预占：参考价加可配置滑点上限、费用 buffer 和 FX buffer，policy version 入库。实际 `consumed_exposure` 超出预占时记录 `overrun_exposure`，reservation 进入 `overrun_locked`，立即触发账户 `OPENING_ORDER_RISK_LOCK` 和 P1 告警；不得通过修改历史 reservation 掩盖超额。`overrun_locked` 的退出与「冲正」必须走 **append-only adjustment event**（补偿事件，绝不修改原成交和原 reservation 历史事实）：
   - 每次调整新增一条 adjustment event，记录 broker fill/event id、调整原因、操作者、调整前/后金额、审批人和证据引用；
   - 原成交记录与原 reservation 永不 UPDATE 历史值，只追加事件；账户额度可随时由事件账本从初始值重放得到当前值；
   - 确认确有超额成交：追加额度补足事件后转 `consumed`；确认超额为重复/错误事件：追加负向 compensation 事件（不删除原成交）后转 `consumed`；确认无超额：追加冲正说明事件后转 `released`；
   - 退出前保持 `overrun_locked` 且账户继续 risk lock，退出动作本身也写入事件表并留审计。
14. 故障注入覆盖：reservation/intent/pending order 原子事务每一步、`submitting` 提交后但网络调用前、broker 已接单但响应丢失、broker 返回后但本地结果事务前、部分成交事件重复/乱序、reconciliation 期间再次崩溃。所有场景验证不会重复下单、重复消费、提前释放或额度泄漏。
15. 旧 QD float 接口只在适配器边界转换，并校验往返误差不超过交易单位。
...
**前置**：P7、P10、P15、P17（§4）；G2 研究门通过。**不依赖 P18 或 G5。**
...
3. **历史迁移边界（与 P15 严格分离）**：paper orders、fills、cash ledger、positions 及其迁移只由 P15 的 canonical ledger 迁移协议处理；P19A **不得**重复导入、不得覆盖、也不得回写 canonical paper ledger。P19A 的导入命令遇到 paper 相关历史数据时直接跳过并引用 P15 的迁移批次，避免两套迁移命令重复处理同一批数据。
4. 无法规范化或租户归属不明确的数据进入隔离错误报告，不猜市场或用户。
5. 正式切换前完成备份恢复演练、增量补录、引用完整性和抽样内容校验。
6. 对照 P4 能力矩阵逐项签字，特别确认 JP/KR/TW、台股三大法人、持续调度、通知和历史查询。
7. 观察至少 **4 周** 的生产并行期，并覆盖月末/月初、周末、节假日以及各目标市场交易日、outcome 到期任务。
8. 指标证明无重复任务、报告、信号、通知后，产出「切换候选」报告。
9. 在 G6 批准**之前**完成停写/恢复演练（消除「回滚演练通过才停 DSA、演练又排在停 DSA 之后」的循环）：
   - 临时停 DSA scheduler 与写 API/通知 → 验证新系统可独立承担写入；
   - 实际恢复 DSA 写路径并验证数据与调度一致；
   - 再次切换到新系统；
   - 演练全程保留只读查询与回滚窗口。
10. 完成上述演练后，提交 G6 人工批准所需的候选材料（能力矩阵签字、并行指标、停写/恢复演练记录、备份与回滚预案）。

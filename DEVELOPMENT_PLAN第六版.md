...
本文件将 `MERGE_DESIGN.md` 拆分为可独立实施、验证、回滚和审批的 Plan。第六版在第五版基础上修复了 broker 提交前后状态持久化时序、accepted 与成交混用、审批订单过早对 worker 可见、真实券商账户身份不稳定、密文调用者迁移不完整等最后一批开发阻断项。各版共同保留的核心方向：
...
| P18 | USStock 受控 live | IBKR/Alpaca 受控实盘 | P14, P16, P17, P0A（安全前置） |
...
5. 建立账户级 `AccountExposureReservationService`（D-013）：
   ...
   - 新增 broker account registry。账户边界使用券商返回的稳定身份 `broker + environment + broker_account_id`，不能只用 `credential_id`；多个 credential 指向同一真实账户时映射到同一 account key，同一 broker account 不得归属多个租户；原始 account id 按敏感字段保存，对外使用不可逆 hash/内部 UUID；
...
6. 创建 intent 的本地原子事务：锁定 signal/binding/broker account/额度行，重新执行关键 eligibility，计算 sizing，预占账户与策略额度，生成唯一 OrderIntent、execution review 和 pending order。live pending order 初始状态必须是 `awaiting_approval` 或 `ready_to_submit`，绝不能直接写成 worker 当前会领取的 `pending`。
...
10. 提交协议（D-016）严格按以下顺序执行：
   ...
   3. `submission_attempt_id` 仅标识本地执行/恢复 attempt，不能作为 broker 幂等键；事务提交成功后才允许调用 broker。同一逻辑订单的所有 attempt、查询和恢复都使用同一个 `client_order_id`，不得在进程重试时重新生成；数据库增加 `(broker_account_key, client_order_id)` 非空唯一约束；
...
12. 市场单/可成交限价单按 worst-case protected price 预占：参考价加可配置滑点上限、费用 buffer 和 FX buffer，policy version 入库。实际 `consumed_exposure` 超出预占时记录 `overrun_exposure`，reservation 进入 `overrun_locked`，立即触发账户 `OPENING_ORDER_RISK_LOCK` 和 P1 告警；不得通过修改历史 reservation 掩盖超额，人工核对并调整额度后才能解除。
...
6. kill switch 关闭后拒绝所有新开仓提交和未开始的 `ready_to_submit`；已经进入 `submission_started/submitting` 的在途网络调用不强行中断，必须按 P14 的 `unknown`/对账协议收敛。kill switch 不能阻断 broker 对账、撤单和可证明不会增加总敞口的确定性减仓；每次决策记录 reason、policy version 和操作者/主体。

...
def create_app(config_name='default', *, register_http_routes: bool = True):
    ...
    # P0A/G0A fail-closed deployment guards (D-009): refuse to boot without
    # a persistent credential key (staging/production), with the bootstrap
    # admin password (staging/production), or with an implicit in-memory
    # key in development. Runs before any route or worker hook.

...
from app.utils.credential_crypto import PURPOSE_MFA_SECRET, decrypt_credential_blob, encrypt_credential_blob
...
class MfaService:
    ...
    def start_setup(self, user_id: int, label: str) -> Dict[str, Any]:
        ...
        encrypted = encrypt_credential_blob(secret, PURPOSE_MFA_SECRET)
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute(
                """
    ...
    def _verify_totp_row(self, row: Dict[str, Any], code: str, update_counter: bool) -> Tuple[bool, str]:
        ...
        secret = decrypt_credential_blob(row.get("secret_encrypted"), PURPOSE_MFA_SECRET)

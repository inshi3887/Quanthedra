...
def get_watcher(chain: str) -> Optional[WatcherFn]:
    ...
    # Mock provider (USDT_PAY_PROVIDER=mock): simulated payments for
    # dev/test/offline CI. Resolved before the real modules load so a mock
    # run never triggers block-explorer HTTP calls; stateless (no registry
    # mutation) so switching back to the live provider cannot leak a stale
    # mock binding.
    from ..mock_provider import resolve_mock_watcher

    mock_fn = resolve_mock_watcher()

...
CHAIN_SPECS: Dict[str, ChainSpec] = {
    ...
    # Simulated chain for USDT_PAY_PROVIDER=mock (ADR-P0-002). Doubly hidden
    # by default: not in USDT_PAY_ENABLED_CHAINS' default list AND no default
    # receiving address, so chain_metadata() returns None until an operator
    # explicitly enables both. With provider=real it resolves no watcher, so
    # a stray MOCK order can stay pending but can never be "confirmed".
    "MOCK": ChainSpec(
        code="MOCK",
        label="Mock (simulated)",
        network_kind="mock",
        decimals=6,
        contract_env="USDT_MOCK_CONTRACT",
        contract_default="mock-usdt-contract",
        address_env="USDT_MOCK_ADDRESS",
        chain_id=0,
        typical_fee_usdt=0.0,
        recommended=False,
        address_prefix_hint="mock",
    ),
}

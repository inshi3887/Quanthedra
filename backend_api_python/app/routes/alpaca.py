...
from app.utils.credential_crypto import PURPOSE_BROKER_CREDENTIAL, decrypt_credential_blob, encrypt_credential_blob
...
def _load_saved_alpaca_config(user_id: int) -> dict:
    ...
    if not row:
        return {}
    try:
        plain = decrypt_credential_blob(row.get("encrypted_config"), PURPOSE_BROKER_CREDENTIAL)
...
def _save_or_update_alpaca_credential(user_id: int, config: AlpacaConfig) -> int:
    ...
    encrypted = encrypt_credential_blob(json.dumps(vault_cfg, ensure_ascii=False), PURPOSE_BROKER_CREDENTIAL)

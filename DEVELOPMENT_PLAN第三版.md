...
本文件将 `MERGE_DESIGN.md` 拆分为可独立实施、验证、回滚和审批的 Plan。第三版在第二版基础上修正了质量门与批准边界的不一致、P0 基础文件缺失、术语不统一等问题。三版共同保留的核心方向：
...
| --- | --- | --- | --- | --- |
| `RESEARCH_DOMAIN_ENABLED` | false | 整个研究域 API 与任务（P8/P9） | G1 通过 | 研究域成为唯一入口且 DSA 退役（P19/G6） |
| `RESEARCH_NEW_DATA_ROUTER_ENABLED` | false | 新 StockDataRouter 成为研究主路径（P5–P7） | P5 双跑达标 | DSA 数据路径停用（P19） |
...
| G2 研究门 | 六市场研究、调度、配额、双跑指标达标 | 研究→订单桥（P16 起）与 DSA 最终退役（P19）；不阻止 P4 的只读 shadow bridge |
...
> 说明：G2「阻止订单桥」特指研究信号进入订单的桥接（P16 `RESEARCH_PAPER_BRIDGE_ENABLED` 起），**不**包括 P4 的只读 DSA shadow bridge。P4 仅依赖 P0，是早期建立对照基线的 Plan，不受 G2 约束。G3 评估的是信号生命周期本身（P11），SignalOutcome（P12）的评估结果不作为 G3 的通过条件，也不被 P13 Eligibility 读取（见 P13 说明）。
...
| P15 | Strategy V2 paper execution | 规范的 paper 成交及账本 | P3, P13, P14 |
...
6. 建立：
   ...
   - ADR 与执行 checklist。
7. 执行前校验上游仓库 HEAD 与 `docs/BASELINE.md` 记录的 commit（QD `e64e1c2`、DSA `396d43a4`）一致；若已漂移，必须更新 BASELINE.md 并记录漂移原因和新 commit，不得在未记录的情况下静默采用新 HEAD。
8. 产品显示层使用 QuantAnalyInvest 名称；QD 商标和内部标识的后续处理单独记录，不混入功能迁移。
...
建议按以下主序列推进（实际依赖以 §4 前置表为准；同一层级的 Plan 在人员充足时可并行分析，但仍应独立交付和审批）：
...
- **G6** 在 P19 后：功能等价、数据迁移、完整观察周期、回滚演练通过。

P19 汇聚 P7、P10、P17、P18 后进行。P1/P2、P3/P4 在人员充足时可并行分析。

开发实施不得跨越对应 Gate：

- **G0 未通过（P0）**，不开始迁移业务代码。
- **G1 未通过（P1/P3 后）**，不开始研究 API 与任务（P8/P9 及之后的研究域写入）。
- **G2 未通过（P5–P10 后）**，不批准研究→订单桥（P16 起）与 DSA 最终退役方案（P19）；P4 的只读 shadow bridge 不受此限制。
...
- **G6 未完成（P19 后）**，不删除 DSA bridge、镜像、备份或迁移映射。

> 注：第二版的批准边界以 Plan 编号描述 Gate（如「P7 未通过」「P10 未通过」），与 §3.2 定义的 G0–G6 编号不一致，且遗漏了 G1/G2/G3 三道门；第三版统一改为 Gate 编号，使每道质量门都有明确的批准边界对应。

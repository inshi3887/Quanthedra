...
本文件将 `MERGE_DESIGN.md` 拆分为可独立实施、验证、回滚和审批的 Plan。第五版在第四版基础上修复了 reservation 状态机内部矛盾、账户级额度被多下单入口并发突破、安全基线可被回滚、G6 与退役顺序循环、以及若干契约未决项。各版共同保留的核心方向：
...
7. 密钥版本化（key-id）：每条已加密凭据记录其使用的密钥版本；新凭据**禁止**使用 `SECRET_KEY` 加密（`credential_crypto.py` 当前在 `CREDENTIAL_ENCRYPTION_KEY` 缺失时回退到 `SECRET_KEY`，必须移除该回退）；旧密文可在受控迁移期用 legacy key 解密并重加密为新 key 版本。
8. 所有进程（API、trading worker、Celery worker、migration job）使用**同一密钥集合**；密钥轮换完成后执行全表校验 + 跨容器 decrypt canary（每个容器解密同一测试密文确认一致）。
...
- 密钥轮换可逆，轮换失败可回退到旧密钥继续解密；全表校验与跨容器 decrypt canary 通过。
...
7. 定义既有 QD 数据库首次接入 migration ledger 的 baseline 策略（**基于 schema fingerprint 严格匹配**）：
   - 为 `init.sql` 生成的 schema 计算 manifest/fingerprint：表名、列名与类型、默认值、主键/外键、唯一约束、索引、函数、触发器、枚举/序列等关键对象；
...
- 两个 runner 并发时只应用一次。
- fresh install 与已有 QD 数据库均通过；已有 QD 数据库首次接入 ledger 时，仅在与 schema manifest 完全匹配时识别为 baseline，不匹配时 fail closed 并产出 reconciliation migration。
...
4. 事务边界与现有 QD 代码兼容（已核实 `order_intents.py` 与 `live_execution.py` 各自 `with get_db_connection()` 开启独立连接）：**直接选方案 A**——改造 `OrderIntentService`、`pending_orders` 写路径等 service 支持**外部传入 connection/cursor**，由 P14 统一事务控制。不采用需要实施者自行补全交易消息语义的 transactional outbox；若未来确需消息化，另拆独立 Plan 定义 outbox 唯一键、claim/lease/fencing、重投幂等、死信与积压告警。事务范围至少覆盖：reservation、execution review、OrderIntent、pending order。
5. 新增 reservation 模型，并建立**账户级 `AccountExposureReservationService`**（D-013）：
   - Research Strategy、原 Strategy V2、Agent quick-trade 三个 live 入口**统一**调用该 service，按 `user + credential/account + currency/market` 锁定账户敞口；
   - 三个入口若分别使用各自额度表，会出现「各自看未超额、合计突破真实账户限制」，必须合并或统一；
   - 统一落地前，Research live 必须使用**独立券商账户**，并禁止与 Agent/原策略并发共用同一账户；
   - 真正创建 intent 时在同一事务中：锁定 signal/binding/额度行 → 重新执行关键 eligibility（不得复用 P13 dry-run 快照，必须在事务内重新求值）→ 计算 sizing → 占用账户和策略额度 → 写唯一 OrderIntent 和 execution review。
6. **事务内禁止行情、券商或账户网络调用**：进入事务前先取得带 `version/as_of` 的确定性快照，事务内只锁定并校验快照版本未变化；版本漂移则回滚并重新快照。
7. reservation 完整状态机与转移表：

   | 当前状态 | 事件 | 下一状态 | 额度动作 |
   | --- | --- | --- | --- |
   | `reserved` | 发起 broker 请求前（尚未离开本进程） | `released` | 释放 |
   | `reserved` | 向 broker 提交成功 | `submitting` | 保持冻结 |
   | `submitting` | 提交超时/进程崩溃/响应解析失败 | `unknown` | 保持冻结（禁止自动释放、禁止自动重试） |
   | `submitting` | broker 明确接受（working/accepted） | `executed` 或 `partial` | 按成交结算 |
   | `unknown` | 对账确认未接单/rejected/cancelled/expired | `released` | 释放 |
   | `unknown` | 对账确认已成交（部分或全部） | `partial`/`executed` | 按成交结算 |
   | `partial` | 剩余部分撤单/拒绝 | `executed`（部分成交剩余终结） | 成交部分转 consumed，剩余释放 |
   | `executed` | 对账确认最终成交 | `consumed` | 不再释放 |
   | `partial`/`executed` | 用户或系统撤单，剩余未成交 | `cancelled` | 未成交部分释放，已成交转 consumed |
   | `reserved`/`submitting`/`unknown`/`partial`（且未成交） | 明确 rejected/cancelled/expired | `released` | 释放 |

   说明：
   - 合法状态为 `reserved / submitting / unknown / partial / executed / consumed / rejected / cancelled / expired / released`，后三者为终态描述；
   - **禁止任何「超时即释放」的泛化规则**：只有「尚未发起 broker 请求且可证明未离开本进程」才允许直接释放；
   - 一旦进入 `submitting`，网络超时、进程崩溃或响应解析失败**统一进入 `unknown`**，绝不因本地超时释放；
   - 仅凭 `client_order_id` 对账确认券商未接单、或明确 rejected/cancelled/expired，才允许释放；
   - `unknown` 必须有：扫描周期、处理 SLA、告警、人工处理权限和审计规则。
8. 重复请求返回同一 intent/reservation；成交后转为 `consumed`，不再释放；部分成交按成交部分结算、剩余撤单释放未成交部分。
9. 补充部分成交、剩余撤单、费用、价格漂移后的额度结算规则：已成交部分按真实成交数量/金额/费用结算并转 consumed，未成交部分按撤单/拒绝结果释放；价格漂移导致的 notional 变化在下次 sizing 时用新快照重算，不回写历史 reservation。
10. 增加「每一步写入后进程崩溃」的故障注入测试：在 reservation 写后、OrderIntent 写后、pending order 写后分别 kill 进程并重启，验证不会出现有 intent 无 reservation、重复占用或额度泄漏。
11. 旧 QD float 接口只在适配器边界转换，并校验往返误差不超过交易单位。

**验收**

- 三个 live 入口（Research、原 Strategy V2、Agent quick-trade）的账户级额度合计不突破真实账户限制；统一前 Research live 使用独立账户且无并发共用。
- 并发请求不能突破账户、策略、每日或单标的额度。
- Decimal 边界、极小值、舍入、费用和 notional drift 测试通过。
- 数据库故障或逐步崩溃时不会出现有 intent 无 reservation 或重复占用。
- `unknown` 状态下额度被冻结，不释放、不自动重试；只有对账确认未接单/rejected/cancelled/expired 才释放；部分成交/剩余撤单/费用/价格漂移结算正确。
...
4. 建立**唯一的 `PaperExecutionService` 与 canonical paper ledger**（fills、cash ledger、positions/snapshots）。本 Plan 开工前必须先完成并批准一份 ADR，固定以下决策，不允许留给实施者自行取舍：
   - canonical paper account/portfolio 的身份定义（单一 identity，Agent 与 Research 共用）；
   - `qd_agent_paper_orders` 历史订单是否迁移、迁移映射与去重策略；
   - 老 Agent API 的兼容期与双写/双读边界；
   - 切换时间点与灰度方式；
   - **禁止长期双写**；旧表何时转只读、何时删除的明确时点；
   - 回滚时以哪个账本为权威（canonical ledger 为唯一事实源）。
   ADR 必须落实：Agent quick-trade 与 Research Strategy 的 paper 都调用同一 `PaperExecutionService`，共享同一现金与持仓事实；若确有隔离需求，只能通过明确的 paper portfolio/account 维度，禁止跨账本汇总风控（eligibility 与 sizing 必须基于 canonical ledger 的单一账本持仓）。
...
1. G6 批准后，先停 DSA scheduler，再停写 API 和通知；保留只读查询及**回滚窗口**。
2. 回滚窗口内继续对账，确认无新的生产事实数据产生；若发现回退，可在窗口内恢复 DSA 写路径（恢复演练已在 P19A 验证）。
3. **回滚窗口结束后**，归档固定 DSA 镜像、配置 schema、迁移映射和运维手册。
4. **回滚窗口结束后**才删除 bridge 和重复实现；删除前确认无引用（任务、信号、通知、订单）。
5. 删除后确认 DSA 不可再产生生产事实数据。

**验收**

- DSA 不再产生生产事实数据，旧数据在统一平台可按租户查询。
- 六市场能力没有未经批准的回退。
- 回滚窗口期间可随时恢复 DSA 写路径；窗口结束后 bridge、镜像、备份与迁移映射均已归档后才删除。
...
- **G6** 在 P19A 切换候选 + 4 周观察 + 停写/恢复演练通过后由人工批准；P19B 正式停写与删除 bridge 在 G6 批准且回滚窗口结束后执行。
...
> 注：第二版的批准边界以 Plan 编号描述 Gate，与 §3.2 定义的 Gate 编号不一致且遗漏 G1/G2/G3；第三版统一为 Gate 编号；第四版新增 G0A，并将 G6 从「退役完成后的确认」修正为「P19A 候选后、P19B 停写前的人工批准」，使 DSA 退役与 US live 解耦；第五版进一步将停写/恢复演练前移到 G6 批准之前、删除 bridge 后移到回滚窗口结束之后，消除「G6 要回滚演练、演练又在 G6 后」的循环。
